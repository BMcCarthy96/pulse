...
export default function EventsPage() {
  ...
  const { items, error, isLoading, mutate, nextCursor, loadMore, loadingMore } =
    usePaginatedList<EventRow>(`/api/v1/events?${params.toString()}`);
  ...
  const columns: Column<EventRow>[] = [
    ...
    { key: "status", header: "Status", render: (r) => <EventStatusBadge status={r.status} /> },
    {
      key: "dedupeKey",
      header: "Dedupe key",
      render: (r) => <code className="text-xs">{r.dedupeKey?.slice(0, 8) ?? "—"}</code>,
    },
          ...
          {selected && (
            <>
              <SheetHeader>
                ...
                <SheetDescription>
                  {selected.connector.displayName} · {selected.direction} ·{" "}
