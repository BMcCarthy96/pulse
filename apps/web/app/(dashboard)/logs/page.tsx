...
export default function LogsPage() {
  ...
  const { items, error, isLoading, mutate, nextCursor, loadMore, loadingMore } = usePaginatedList<LogRow>(
    live ? `/api/v1/logs?${params.toString()}` : null,
  );
  ...
  const columns: Column<LogRow>[] = [
    ...
    { key: "message", header: "Message", render: (r) => <span className="line-clamp-1">{r.message}</span> },
          ...
          {selected && (
            <>
              <SheetHeader>
                ...
                <SheetDescription>
                  {selected.level} · {selected.source} · {selected.connector?.displayName ?? "no connector"}
                </SheetDescription>
              </SheetHeader>
